...
async def agent_chat(
    ...
    async def agent_chat_iterator() -> AsyncIterable[str]:
        ...
        code_guide = code_guide = """
                    CRITICAL RULE FOR CODE INTERPRETER:
                    - You MUST call code interpreter SEPARATELY for each chart you want to create.
                    - Each code block MUST contain ONLY ONE plt.show() call.
                    - NEVER put multiple plt.show() calls in the same code block.
                    - If you need to create 2 charts, you MUST call code interpreter 2 times.
                    - Example for 2 charts:
                      1st call: plot first chart with plt.show()
                      2nd call: plot second chart with plt.show()
                    """
        full_sys_prompt = sys_prompt + code_guide
        system_prompt = SystemMessagePromptTemplate.from_template(full_sys_prompt)
        ...
        # 最后将输出文件列表发送
        if output_files:
            images_markdown = "\n\n".join([f"![](http://localhost:6605{img})" for img in output_files.split("\n\n")])
            yield json.dumps({"answer": f'\n\n{output_codes}\n\n{images_markdown}'}).encode('utf-8')
        else:
